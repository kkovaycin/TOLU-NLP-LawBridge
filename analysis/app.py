...
# --- Purity kuralı (deterministik) -----------------
DEFAULT_SENTIMENT = "Olumsuz"
DEFAULT_PURITY    = "uncertain"   # etiketsiz/boş metin için

def purity_from_legal(legal_labels: list[str]) -> str:
    """
    Kural:
    - Hiç label yoksa: 'uncertain'
    - Sadece 'Uygunsuzluk Yok' varsa: 'good_faith'
    - Herhangi bir ihlal label'i varsa: 'bad_faith'
    """
    labels = [str(l).strip() for l in (legal_labels or []) if str(l).strip()]
    if not labels:
        return DEFAULT_PURITY
    if any(l != "Uygunsuzluk Yok" for l in labels):
        return "bad_faith"
    return "good_faith"
...
YOUTUBE_API_KEY = os.getenv("YOUTUBE_API_KEY", "youtube_api_key").strip()
...
def _maybe_label_with_model(df: pd.DataFrame, text_col: Optional[str]) -> pd.DataFrame:
    ...
    for t in df[text_col].fillna("").tolist():
        # 1) Legal
        # 1) Legal
        legal_list = classify_multi(clf_legal, t, LEGAL_MAXLEN, LEGAL_THRESH, MAX_LABELS_PER_DIM) or []

# İhlal varsa 'Uygunsuzluk Yok' etiketi asla kalmasın
        if legal_list:
            if any(lbl != "Uygunsuzluk Yok" for lbl in legal_list):
                legal_list = [lbl for lbl in legal_list if lbl != "Uygunsuzluk Yok"]

# Hâlâ boşsa tek seçenek olarak 'Uygunsuzluk Yok'
        if not legal_list:
